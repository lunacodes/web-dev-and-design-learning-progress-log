...
The per-repo config can be found in `.git/config` - very useful!
...
```bash
git config trailer.sign.key "Etc-Signed-off-by: "
export SIGNOFF="sign: $(git config --get user.name) <$(git config --get user.email)>"
git filter-branch -f --msg-filter     "git interpret-trailers --trailer \"$SIGNOFF\""      HEAD~1..HEAD
```
